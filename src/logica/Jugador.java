...
public class Jugador extends AColega implements IJugador, IDibujable, IDetenedor,IComponente {
	...
	public void iniciar() {
		...
		while (!((jf = jf.getParent()) instanceof JFrame));
                start = java.applet.Applet.newAudioClip(getClass().getResource("/agregados/341695__projectsu012__coins-1.wav"));
		ingame = java.applet.Applet.newAudioClip(getClass().getResource("/agregados/516912__xythe__chill-tune-for-a-game.wav"));
                start.play();
                ingame.play();
	...
	public void detener() {
		Container jf = (JPanel) juego;
		while (!((jf = jf.getParent()) instanceof JFrame));
                pause = java.applet.Applet.newAudioClip(getClass().getResource("/agregados/415083__harrietniamh__video-game-coin.wav"));
		pause.play();
                ingame.stop();
